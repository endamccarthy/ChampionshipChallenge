...
def get_leaderboard_page(request):
...
@login_required
def get_checkout_page(request, entry_id):
  stripe.api_key = settings.STRIPE_SECRET_KEY
  session = stripe.checkout.Session.create(
      payment_method_types=['card'],
      line_items=[{
          'price': 'price_1H4ocuKSkIdC4LCMnGWYJwGx',
          'quantity': 1,
      }],
      mode='payment',
...
def get_entry_page(request, entry_id):
  try:
...
def get_user_entries_page(request):
